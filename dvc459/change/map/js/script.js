...
//});
	...
	dvc.time = "yr" + config.ons.nonStdScale.length;
...
//			  topoLayer.addTo(map);
	...
	function enterLayer(){ //console.log("rollover");  console.log(this);
		currclass = d3.select(this).attr("class").split(' ')[0];
		//console.log("enterLayer" + currclass);
		d3.select('#selected').remove();
		highlightArea();
    highlightKey(this);
	}

    function highlightArea(){ //console.log("highlightArea");
    ...
    if(rateById[currclass] > 0) {dvc.prefix = "+"; } else {dvc.prefix = ""; };
...
//			 }
 	...
 	d3.select(".x").selectAll("text").each(function(d,i){
					//console.log(d) 
					if (i==0){
						d3.select(this).text("02/07")
					} else if(i==1){
						d3.select(this).text("")
					} else if(i==2){
						d3.select(this).text("06/11")
					 }else if(i==3){
						d3.select(this).text("")
					} else{
					d3.select(this).text("10/15")
					}
	});
